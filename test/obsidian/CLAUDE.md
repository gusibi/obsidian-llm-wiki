...
log.md           → 仅追加（append-only）的时间顺序操作日志。
```
...
页面类型：
- **摘要 (Summary)** (`wiki/summaries/`)：每个摄取的源一个。捕捉要点、背景和相关性。
- **概念 (Concept)** (`wiki/concepts/`)：每个重要概念或主题一个。综合多个源的信息。**正文中每段新增内容必须标注来源**。
- **实体 (Entity)** (`wiki/entities/`)：每个著名人物、工具、框架、组织一个。**正文中每段新增内容必须标注来源**。
...
**Summary 页面**：一对一映射 raw 文件，sources 字段指向对应的 raw 源文件名。
...
示例：假设 ingest 一篇叫 `attention-is-all-you-need.md` 的 raw 文件，你在 `wiki/summaries/` 下创建了 `attention-is-all-you-need.md` 作为摘要：
...
注意上面的引用 `[[attention-is-all-you-need]]` 就是 `wiki/summaries/attention-is-all-you-need.md` 的实际文件名。**不是** `[[summary-attention-is-all-you-need]]`——除非你的 summary 文件确实命名为 `summary-attention-is-all-you-need.md`。
...
4. 追加到 `log.md`：
   ...
   - Pages created: [[实际创建的文件名1]], [[实际创建的文件名2]]
   - Pages updated: [[实际更新的文件名1]], [[实际更新的文件名2]]
...
6. 如果归档，请更新 `index.md` 并追加到 `log.md`。
7. 无论是否归档，都追加到 `log.md`（query 类型条目）：
   ```
   ## [YYYY-MM-DD] query | 问题摘要
   - Question: 用户的原始问题
   - Wiki pages consulted: [[page1]], [[page2]]
   - Filed as: [[实际的文件名]]（如未归档则写 "not filed"）
   ```
...
输出检查报告，修复你能修复的内容，并为其余部分建议操作。追加到 `log.md`。
...
11. **每次 query 都记录到 log.md**，无论回答是否被归档为 wiki 页面。
